...
gamma = c( (1/dim(data)), (1/(1.1*(dim(data)))), (1/(1.3*(dim(data)))), (1/(1.5*(dim(data)))), (1/(1.7*(dim(data)))) )
    ...
    projected.test = t(t(Eigenfaces)%*%t(test.scaled))
      ...
      if (k != "linear"){
        for (g in gamma){
          ...
          classifier=svm(formula=train.labels ~. ,data=projected.train,
          ...
          predict_v = predict(classifier, newdata = projected.test)
